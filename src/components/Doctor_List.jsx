...
export const Doctor_List = () => {
  const navigate=useNavigate();
    ...
    const handleSignup = async (username) => {
        ...
        const response = await axios.get(`https://hospital-backend-2ox9.onrender.com/doctorlist?search=${hospitalname}`).then((response) => {
      ...
      const assignappoint = async (username) => {
        ...
        const response = await axios.get(`https://hospital-backend-2ox9.onrender.com/appointment?param1=${username}&param2=${patientname}`).then((response) => {
          console.log(response.data);
          let arr=response.data;
